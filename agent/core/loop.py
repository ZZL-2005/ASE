...
from agent.core.memory import MemoryConsolidator
from agent.core.runner import AgentRunSpec, AgentRunner
from agent.core.subagent import SubagentManager
from agent.core.tools.cron import CronTool
from agent.core.skills import BUILTIN_SKILLS_DIR
from agent.core.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from agent.core.tools.message import MessageTool
from agent.core.tools.registry import ToolRegistry
from agent.core.tools.shell import ExecTool
from agent.core.tools.spawn import SpawnTool
from agent.core.tools.web import WebFetchTool, WebSearchTool
...
class _LoopHookChain(AgentHook):
    ...
    async def get_forced_response(self, context: AgentHookContext):
        result = await self._extras.get_forced_response(context)
        if result is not None:
            return result
        return await self._primary.get_forced_response(context)

    def get_forced_tool_result(self, tool_name: str, params: dict):
        result = self._extras.get_forced_tool_result(tool_name, params)
        if result is not None:
            return result
        return self._primary.get_forced_tool_result(tool_name, params)
...
class AgentLoop:
    ...
    async def _connect_mcp(self) -> None:
        ...
        from agent.core.tools.mcp import connect_mcp_servers
        try:
