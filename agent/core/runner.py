...
class AgentRunner:
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        for iteration in range(spec.max_iterations):
            ...
            if hook.wants_streaming():
                ...
                # Check if hook provides a forced response (replay mode)
                forced = await hook.get_forced_response(context)
    ...
    async def _run_tool(
        ...
        hook = spec.hook or AgentHook()
        # Check if hook provides a forced result (replay mode)
        forced = hook.get_forced_tool_result(tool_call.name, tool_call.arguments)
        if forced is not None:
            return forced, {"name": tool_call.name, "status": "ok", "detail": "(replayed)"}, None
